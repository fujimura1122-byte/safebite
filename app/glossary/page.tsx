...
const dangerLabel: Record<string, { label: string; cls: string }> = {
      ...
      <main className="max-w-3xl mx-auto px-4 py-12">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-gray-500 text-lg leading-relaxed mb-6">
            「UD」「ホワ案」「受け子」など、<br />
            SNSで飛び交う<strong className="text-gray-800">闇バイトの隠語</strong>をすべて解説します。
          </p>
          <div className="bg-red-50 border-2 border-red-200 rounded-2xl p-5 text-sm text-red-700 leading-relaxed text-left">
            <strong className="block mb-2">⚠️ これらの言葉を使った求人には絶対に応募しないでください</strong>
            隠語でカモフラージュされた「バイト」の実態は犯罪です。
            知らずに関与した場合でも逮捕・起訴される事例が多数報告されています。
          </div>
        </div>
          ...
          {terms.map((t) => {
            ...
            return (
              <article
                ...
                id={t.word}
              >
                <div className="bg-gray-50 border-b border-gray-100 px-6 py-4 flex flex-wrap gap-3 items-center">
                  <h2 className="text-xl font-black text-gray-900">{t.word}</h2>
                  <span className="text-sm text-gray-400">（{t.read}）</span>
                  <span className={"text-xs font-bold px-3 py-1 rounded-full border " + d.cls}>
                    {d.label}
                  </span>
                </div>
                <div className="px-6 py-5">
                  <p className="text-base font-bold text-gray-800 mb-3">{t.short}</p>
                  <p className="text-sm text-gray-600 leading-relaxed mb-4">{t.detail}</p>
                  <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 mb-4">
                    <div className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">勧誘文の例</div>
                    <p className="text-sm text-gray-700 italic">{t.example}</p>
                  </div>
                  <div className="flex gap-2 items-start">
                    <span className="text-red-500 font-bold text-sm flex-shrink-0">罰則：</span>
                    <span className="text-sm text-red-600 font-bold">{t.punishment}</span>
                  </div>
                </div>
              </article>
            );
        ...
        <div className="mt-12 bg-red-50 border-2 border-red-200 rounded-2xl p-8 text-center">
          ...
          <p className="text-gray-500 mb-6 text-sm leading-relaxed">
            SafeBiteのAI危険度チェッカーで<br />
            すぐに分析できます（無料・匿名）
          </p>
          <Link
            href="/#checker"
            className="inline-block bg-red-500 hover:bg-red-600 text-white font-bold px-8 py-4 rounded-xl text-lg transition-colors"
          >
            無料で危険度チェックする →
          </Link>
          <p className="text-xs text-gray-400 mt-3">入力した文章はサーバーに保存されません</p>
        </div>

        {/* Index */}
        <div className="mt-10 bg-gray-50 border border-gray-200 rounded-2xl p-6">
          <div className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">この辞典で解説している隠語一覧</div>
          <div className="flex flex-wrap gap-2">
            {terms.map((t) => (
              <a
                key={t.word}
                href={"#" + t.word}
                className="text-xs font-bold bg-white border border-gray-200 hover:border-red-300 text-gray-600 hover:text-red-500 px-3 py-1.5 rounded-lg transition-colors"
              >
                {t.word}
              </a>
            ))}
